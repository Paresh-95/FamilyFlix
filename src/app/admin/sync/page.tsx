...
import { RefreshCw, CheckCircle2, AlertTriangle, Search, Plus, ChevronLeft, Clapperboard, Tv } from 'lucide-react';
...
export default function SyncDrivePage() {
  ...
  async function addItem(candidate: Candidate, tmdbId: number): Promise<boolean> {
    if (candidate.type === 'movie') {
      const res = await fetch('/api/movies', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ driveFileId: candidate.driveId, tmdbId }),
      });
      ...
      const res = await fetch('/api/series', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ driveFolderId: candidate.driveId, tmdbId }),
      });
      return res.ok || res.status === 409;
    }
  }
  ...
  const sync = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          const ok = await addItem(c, c.tmdbMatches[0].id);
          if (ok) {
            newAdded.push({ filename: c.filename, title: c.tmdbMatches[0].title, poster_path: c.tmdbMatches[0].poster_path, type: c.type });
          } else {
  ...
  function updateConflict(driveId: string, patch: Partial<ConflictRow>) {
    setConflicts((prev) => prev.map((r) => r.candidate.driveId === driveId ? { ...r, ...patch } : r));
  }

  async function searchTmdb(driveId: string, query: string, type: 'movie' | 'series') {
    updateConflict(driveId, { searching: true });
    try {
      const endpoint = type === 'series' ? `/api/tmdb-tv-search?q=${encodeURIComponent(query)}` : `/api/tmdb-search?q=${encodeURIComponent(query)}`;
      const res     = await fetch(endpoint);
      const results = await res.json();
      updateConflict(driveId, { searchResults: results, searching: false });
    } catch {
      updateConflict(driveId, { searching: false });
    }
  }

  async function addConflict(row: ConflictRow) {
    if (!row.selected) return;
    updateConflict(row.candidate.driveId, { status: 'adding' });
    try {
      const ok = await addItem(row.candidate, row.selected.id);
      if (ok) {
        setAdded((prev) => [...prev, { filename: row.candidate.filename, title: row.selected!.title, poster_path: row.selected!.poster_path, type: row.candidate.type }]);
        updateConflict(row.candidate.driveId, { status: 'added' });
      } else {
        updateConflict(row.candidate.driveId, { status: 'error' });
      }
    } catch {
      updateConflict(row.candidate.driveId, { status: 'error' });
    }
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/30 text-sm mt-0.5">Auto-detect new movies and series, add them instantly</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 md:px-12 pb-24 space-y-6 max-w-5xl mx-auto">
        ...
        <div className="bg-white/[0.03] border border-white/8 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5">
          <div className="flex-1">
            ...
            <p className="text-white/35 text-sm mt-1 leading-relaxed">
              Finds new video files <span className="text-white/50">(movies)</span> and subfolders <span className="text-white/50">(series)</span> in Drive,
              matches them on TMDB automatically, and adds everything in one go.
            </p>
          </div>
          <button
            onClick={sync}
        ...
        {scanError && (
          <div className="flex items-center gap-3 bg-netflix-red/8 border border-netflix-red/25 text-red-400 px-5 py-4 rounded-xl text-sm">
            <AlertTriangle size={16} className="shrink-0" />{scanError}
          </div>
        )}
        ...
        {scanned && added.length === 0 && conflicts.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 border border-white/5 rounded-2xl gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-white/35 text-sm mt-1">No new movies or series found in Drive.</p>
            </div>
          </div>
        )}
          ...
          <section className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-amber-400 text-sm font-semibold">
                Needs attention — {pendingConflicts.length} item{pendingConflicts.length !== 1 ? 's' : ''} couldn&apos;t be matched
              </p>
            </div>
            {conflicts.map((row) => {
              if (row.status === 'added') return null;
              const Icon = row.candidate.type === 'series' ? Tv : Clapperboard;
              return (
                <div key={row.candidate.driveId} className="bg-white/[0.03] border border-amber-500/15 rounded-2xl p-5 space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${row.candidate.type === 'series' ? 'bg-teal-500/15 text-teal-400' : 'bg-blue-500/15 text-blue-400'}`}>
                          {row.candidate.type === 'series' ? 'Series' : 'Movie'}
                        </span>
                    ...
                    >
                      {row.status === 'adding' ? <span className="w-3 h-3 border border-white/30 border-t-white rounded-full animate-spin" /> : <Plus size={13} />}
                      Add
                    </button>
                  </div>
                  <div className="flex gap-2">
                    <input
                      type="text" value={row.searchQuery}
                      onChange={(e) => updateConflict(row.candidate.driveId, { searchQuery: e.target.value })}
                      onKeyDown={(e) => e.key === 'Enter' && searchTmdb(row.candidate.driveId, row.searchQuery, row.candidate.type)}
                      placeholder={`Search TMDB ${row.candidate.type === 'series' ? 'TV shows' : 'movies'}…`}
                    ...
                    />
                    <button
                      onClick={() => searchTmdb(row.candidate.driveId, row.searchQuery, row.candidate.type)}
                    ...
                    >
                      <Search size={14} />{row.searching ? '…' : 'Search'}
                    </button>
                  </div>
                  {row.searchResults.length > 0 && (
                    <div className="rounded-xl border border-white/8 overflow-hidden" style={{ background: '#0e0e0e' }}>
                      {row.searchResults.map((r) => (
                        <button key={r.id}
                          onClick={() => updateConflict(row.candidate.driveId, { selected: r, searchResults: [] })}
        ...
        {added.length > 0 && (
          <section className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-emerald-400 text-sm font-semibold">{added.length} item{added.length !== 1 ? 's' : ''} added successfully</p>
            </div>
            <div className="space-y-2">
              {added.map((m, i) => (
                <div key={i} className="flex items-center gap-3 bg-emerald-500/[0.04] border border-emerald-500/15 rounded-xl px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${m.type === 'series' ? 'bg-teal-500/15 text-teal-400' : 'bg-blue-500/15 text-blue-400'}`}>
